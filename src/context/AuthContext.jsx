...
const AuthProvider = ({ children }) => {
  ...
  const login = async (idToken) => {
    ...
    const logout = async () => {
      ...
      await axios.post(`${import.meta.env.VITE_SERVER_BASE_URL}/api/auth/logout`, { refreshToken });
    };
      ...
      const interceptor = axios.interceptors.response.use(
        ...
        async (error) => {
          ...
          if (error.response.status === 401 && !originalRequest._retry) {
            ...
            originalRequest.headers['Authorization'] = `Bearer ${localStorage.getItem('authTokens')?.accessToken}`;
